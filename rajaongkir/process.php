			...
			foreach($results as $r):
				foreach($r->costs as $rc):
					foreach($rc->cost as $rcc):
						echo "<tr><td>$r->code</td><td>$rc->service</td><td>$rc->description</td><td>$rcc->etd</td><td>".number_format($rcc->value)."</td><td><input type=radio name=pilih class=pilih[] value=$rcc->value onclick=getValue(); required></td></tr>";
					endforeach;
				endforeach;
			endforeach;
		endif;
