...
function loadProvinsi(id){
	...
	$(id).html('loading...');
	$.ajax({
		url:'http://localhost/e/rajaongkir/process.php?act=showprovince',
		dataType:'json',
...
function loadCity(idprovince,id){
	$.ajax({
		url:'http://localhost/e/rajaongkir/process.php?act=showcity',
		dataType:'json',
...
function cekHarga(){
	...
	var origin = 151; //masukan bekasi
	var destination = $('#descity').val();
	var weight = $('#berat').val();
	var courier = $('#service').val();
	$.ajax({
		url:'http://localhost/e/rajaongkir/process.php?act=cost',
